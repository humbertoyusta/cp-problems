    /**  Solucion oficial aceptada de la subtarea 2 -- Complejidad O(n+T*sqrt(n)) */
#include<bits/stdc++.h>
using namespace std;
/// Pragmas
...
int tc, phi[maxn], pr[maxn], prpw[maxn], pot[(int)sqrt(maxn)+11][25];
...
int32_t main(){
    ...
    while( tc-- ){
        ...
        if( v[0] == v[1] ){
            ...
            ans -= phi[m/v[0]] * ( phi[m/v[0]] + 1 ) / 2;
            ans -= phi[m/v[1]] * ( phi[m/v[1]] + 1 ) / 2;
            ans -= ( m - phi[m] );
        }
        cout << ans << '\n';
    }
